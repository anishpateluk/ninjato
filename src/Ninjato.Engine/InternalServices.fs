...
module ExpressionEvaluation = 
    ...
    let private ``remove square brackets`` (token: string) =
        token.Replace("[]","")
...
module QueryConstruction =
    ...
    let private ``evaluate construct tree`` (tree: list<Node>) (csql: string) (argMap: Map<string, string>) =
        let ``extract content`` (start: int) (stop: int) = 
            csql.Substring(start, stop - start)

        let rec traverse (str: string) (sb: StringBuilder) (``previous evaluation``: bool) (``pre content start``: int) (tree: list<Node>) (argMap: Map<string, string>) =
            match tree with
                | [] -> sb
                | head :: tail ->
        ...
        Seq.fold(fun (tup: int*string*Map<string, Object>) (kvp: KeyValuePair<string, string>) -> 
                        ...
                        if str.Contains(kvp.Key) then
                            if kvp.Key.Contains("[]") then
                                ...
                                let args = query { for a in ``untrimmed args`` do
                                                   let arg = a.Trim()
                                                   select arg }
                                           |> Seq.mapi(fun index arg -> (sprintf "@p%i" (i + index), box arg))
                                           |> Map.ofSeq

                                let replacement = query { for kvp in args do select kvp.Key } |> Array.ofSeq |> fun arr -> String.Join(",",arr)
                                
                                ((i + args.Count), str.Replace(kvp.Key, replacement), args)
                            else
                                ((i + 1), str.Replace(kvp.Key, sprintf "@p%i" i), map.Add(sprintf "@p%i" i, box kvp.Value))
                        else
                            tup
                        ) 
        <| (1, sql, Map.empty) <| argMap
        |> fun tup ->
            let i, str, map = tup
            ...
            |> Seq.map(fun m ->
                match m with
                    | x when x.Groups.["if"].Success -> 
                        If({ Index = m.Index; Length = m.Length; Value = m.Value })
                    | x when x.Groups.["else"].Success -> 
                        Else({ Index = m.Index; Length = m.Length; Value = m.Value })
                    | x when x.Groups.["end"].Success -> 
                        End({ Index = m.Index; Length = m.Length; Value = m.Value })
                    | _ -> failwith "Construct not recognized"
                )
            |> ``build tree`` <| ``number of constructs``
            |> ``evaluate construct tree`` <| str <| argMap
